...
class ProviderTest(TestCase):
    ...
    def test_patch(self):
        response = self.client.patch(
            reverse('provider_detail', kwargs={'pk': 2}), {'name': 'new_name'}, content_type='application/json')
        self.assertEqual(response.status_code, 200)

        response = self.client.get(
            reverse('provider_detail', kwargs={'pk': 2}))
    ...
    def test_put(self):
        response = self.client.put(
            reverse('provider_detail', kwargs={'pk': 4}), {'email': 'new@gmail.com'}, content_type='application/json')
        self.assertEqual(response.status_code, 200)

        response = self.client.get(
            reverse('provider_detail', kwargs={'pk': 4}))
